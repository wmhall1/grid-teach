...
def plot_S_contour(csv_file, output_file=None, levels=20, log=False):
    """Make a contour plot of lowest S values in temperature/mass space, or a line plot if 1D."""
    ...
    # --- Group by (temp, mass) and take the minimum S per group ---
    df_grouped = (
        df.groupby(["temp", "mass"], as_index=False)
          .agg({"S": "min"})
    )

    temp = df_grouped["temp"].values
    mass = df_grouped["mass"].values
    S = df_grouped["S"].values
    ...
    if len(unique_mass) < 3 or len(unique_temp) < 3:
        ...
        plt.ylabel("S value (min per T/M)")
    ...
    plt.scatter(temp[best_idx], mass[best_idx], s=100, c="red", edgecolor="white",
                marker="*", label=f"Best Fit (S = {np.min(S):.4f})")

    plt.xlabel("Effective Temperature (K)", fontsize=12)
    plt.ylabel("Mass ($M_\\odot$)", fontsize=12)
    plt.title("Best-fit (min S) in Temperature–Mass Space", fontsize=14)
    plt.legend()
    plt.tight_layout()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot S contour from fit results (min S per T/M).")
