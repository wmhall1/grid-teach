...
def get_adj(filename):
    ...
    data= data.reset_index()
    run = data.loc[0]
    df = pd.DataFrame(index = data.index, columns = ["name", "temp", "mass", "h", "he", "S", "mem", "per"])
    for i in np.arange(0,len(data)):
        run = data.loc[i]
        df.name[i] = run[0]
        df.temp[i] = float(run[1])
        df.mass[i] = float(run[2])
        df.h[i] = float(run[3][0:5]) * 0.01
        df.he[i] = float(run[3][6:12]) * 0.01
        df.S[i] = float(run[4])
        df.mem[i] = float(run[5])
        df.per[i] = float(run[6])


    return(df) 

def get_wei(filename):
    data = pd.read_table(filename,sep = " & ", header=None, names = ["name", "temp", "mass", "hy", "he", "S"], dtype = {'name':str,'temp':float,'mass':float,'hy':float,'he':float,'S':float})
    return(data)
