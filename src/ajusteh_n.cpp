...
int main(int argc, char *argv[]){
    ...
    ofstream output, outputper, doutput, modeput;
    ...
    */
    ...
    for(int ifi=0; ifi<starcount; ifi++){
        ...
        if(!goto9000){
            ...
            for(int i = 0; i < np; i++){
                modeput << setprecision(2) << fixed << modecs[i] << "("  << setprecision(0) << fixed << limposed[i] << "," << ks[i] <<")";
                if(i != np -1){
                    modeput << ", ";
                }
            }
