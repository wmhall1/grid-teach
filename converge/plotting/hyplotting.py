...
title = "T = " + str(userTemp) + "K, M = " + str(userMass) + "$M_o$, log($M_{He}$/$M_o$) = -" + str(userMassHe) 
...
for line in lines:
    ...
    if temp == float(userTemp) and mass == float(userMass) and masshe == float(userMassHe):
        total = total+1
        titles.append(linenumber)
        mh.append(-1 * massh/100)
        t.append(temp)
        if debug:
            print(temp)
            print(mass)
...
plt.figure(100)
plt.plot(mh,p1,"-", label = "k = 1")
plt.plot(mh,p2,"--", label = "k = 2")
plt.plot(mh,p3,"-.", label = "k = 3")
plt.plot(mh,p4,":", label = "k = 4")
